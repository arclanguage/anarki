...
(define (*read-syntax src port)
  ...
  (current-namespace arc-ns)

(define _sig fn-signatures)
(define _apply (lambda (fn . args) (ar-apply fn (ar-apply-args args))))
(define _cons cons)
(define _car (lambda (x) (cond ((pair? x) (car x)) ((eqv? x (quote nil)) (quote nil)) ((eqv? x (quote ())) (quote nil)) (#t (err "Can't take car of" x)))))
(define _cdr (lambda (x) (cond ((pair? x) (cdr x)) ((eqv? x (quote nil)) (quote nil)) ((eqv? x (quote ())) (quote nil)) (#t (err "Can't take cdr of" x)))))
(define _is (lambda args (tnil (or (all (lambda (a) (eqv? (car args) a)) (cdr args)) (and (all string? args) (apply string=? args)) (all ar-false? args)))))
(define _err err)
(define _nil (quote nil))
(define _t (quote t))
(define _+ (lambda args (cond ((null? args) 0) ((all string? args) (apply string-append args)) ((all arc-list? args) (ac-niltree (apply append (map ar-nil-terminate args)))) (#t (apply + args)))))
(define _- -)
(define _* *)
(define _/ /)
(define _mod modulo)
...
(define _expt expt)
(define _sqrt sqrt)
(define _> (lambda args (tnil (apply arc> args))))
(define _< (lambda args (tnil (apply arc< args))))
(define _len (lambda (x) (cond ((string? x) (string-length x)) ((vector? x) (vector-length x)) ((hash-table? x) (hash-table-count x)) (#t (length (ar-nil-terminate x))))))
(define _annotate ar-tag)
(define _type ar-type)
(define _rep ar-rep)
(define _uniq ar-gensym)
(define _ccc call-with-current-continuation)
(define _infile open-input-file)
(define _outfile (lambda (f . args) (open-output-file f (quote text) (if (equal? args (quote (append))) (quote append) (quote truncate)))))
(define _instring open-input-string)
(define _outstring open-output-string)
(define _inside get-output-string)
(define _close (lambda args (map (lambda (p) (cond ((input-port? p) (close-input-port p)) ((output-port? p) (close-output-port p)) ((tcp-listener? p) (tcp-close p)) (#t (err "Can't close " p)))) args) (quote nil)))
(define _stdout current-output-port)
(define _stdin current-input-port)
(define _stderr current-error-port)
(define _call-w/stdout (lambda (port thunk) (parameterize ((current-output-port port)) (thunk))))
(define _call-w/stdin (lambda (port thunk) (parameterize ((current-input-port port)) (thunk))))
(define _readc (lambda (str) (let ((p (if (ar-false? str) (current-input-port) str))) (let ((c (read-char p))) (if (eof-object? c) (quote nil) c)))))
(define _readb (lambda (str) (let ((p (if (ar-false? str) (current-input-port) str))) (let ((c (read-byte p))) (if (eof-object? c) (quote nil) c)))))
(define _peekc (lambda (str) (let ((p (if (ar-false? str) (current-input-port) str))) (let ((c (peek-char p))) (if (eof-object? c) (quote nil) c)))))
(define _writec (lambda (c . args) (write-char c (if (pair? args) (car args) (current-output-port))) c))
(define _writeb (lambda (b . args) (write-byte b (if (pair? args) (car args) (current-output-port))) b))
(define _write (lambda args (let ((port (if (pair? (cdr args)) (cadr args) (current-output-port)))) (if (pair? args) (write (ac-denil (car args)) port)) (flush-output port)) (quote nil)))
(define _disp (lambda args (let ((port (if (pair? (cdr args)) (cadr args) (current-output-port)))) (if (pair? args) (display (ac-denil (car args)) port)) (flush-output port)) (quote nil)))
(define _sread (lambda (p eof) (let ((expr (read p))) (if (eof-object? expr) eof expr))))
(define _coerce (lambda (x type . args) (cond ((ar-tagged? x) (err "Can't coerce annotated object")) ((eqv? type (ar-type x)) x) ((char? x) (case type ((int) (char->ascii x)) ((string) (string x)) ((sym) (string->symbol (string x))) (else (err "Can't coerce" x type)))) ((integer? x) (case type ((char) (ascii->char x)) ((string) (apply number->string x args)) (else (err "Can't coerce" x type)))) ((number? x) (case type ((int) (round x)) ((char) (ascii->char (round x))) ((string) (apply number->string x args)) (else (err "Can't coerce" x type)))) ((string? x) (case type ((sym) (string->symbol x)) ((cons) (ac-niltree (string->list x))) ((int) (or (apply string->number x args) (err "Can't coerce" x type))) (else (err "Can't coerce" x type)))) ((pair? x) (case type ((string) (list->string (ar-nil-terminate x))) (else (err "Can't coerce" x type)))) ((eqv? x (quote nil)) (case type ((string) "") (else (err "Can't coerce" x type)))) ((symbol? x) (case type ((string) (symbol->string x)) (else (err "Can't coerce" x type)))) (#t x))))
(define _open-socket (lambda (num) (tcp-listen num 50 #t)))
(define _socket-accept (lambda (s) (call-with-values (lambda () (tcp-accept s)) (lambda (in out) (list (make-limited-input-port in 100000 #t) out (let-values (((us them) (tcp-addresses out))) them))))))
(define _new-thread thread)
(define _kill-thread kill-thread)
(define _break-thread break-thread)
(define _sleep (wrapnil sleep))
(define _system (wrapnil system))
(define _pipe-from (lambda (cmd) (let ((tf (ar-tmpname))) (system (string-append cmd " > " tf)) (let ((str (open-input-file tf))) (system (string-append "rm -f " tf)) str))))
(define _vec (lambda (n) (make-vector n (quote nil))))
(define _vec-ref vector-ref)
(define _vec-set vector-set!)
(define _table (lambda () (make-hash-table (quote equal))))
(define _maptable (lambda (fn table) (hash-table-for-each table fn) table))
(define _protect (lambda (during after) (dynamic-wind (lambda () #t) during after)))
(define _rand random)
(define _dir (lambda (name) (map path->string (directory-list name))))
(define _file-exists (lambda (name) (if (file-exists? name) name (quote nil))))
(define _dir-exists (lambda (name) (if (directory-exists? name) name (quote nil))))
(define _rmfile (wrapnil delete-file))
(define _macex (lambda (e) (ac-macex (ac-denil e))))
(define _macex1 (lambda (e) (ac-macex (ac-denil e) (quote once))))
(define _eval (lambda (e) (eval (ac (ac-denil e) (quote ())) arc-ns)))
(define _on-err on-err)
(define _details (lambda (c) (disp-to-string (exn-message c))))
(define _scar (lambda (x val) (if (string? x) (string-set! x 0 val) (set-car! x val)) val))
(define _scdr (lambda (x val) (if (string? x) (err "Can't set cdr of a string" x) (set-cdr! x val)) val))
(define _sref (lambda (com val ind) (cond ((hash-table? com) (if (eqv? val (quote nil)) (hash-table-remove! com ind) (hash-table-put! com ind val))) ((vector? com) (vector-set! com ind val)) ((string? com) (string-set! com ind val)) ((pair? com) (nth-set! com ind val)) (#t (err "Can't set reference " com ind val))) val))
(define _bound (lambda (x) (tnil (bound? x))))
(define _newstring make-string)
(define _trunc (lambda (x) (inexact->exact (truncate x))))
(define _exact (lambda (x) (tnil (and (integer? x) (exact? x)))))
(define _msec current-milliseconds)
(define _current-process-milliseconds current-process-milliseconds)
(define _current-gc-milliseconds current-gc-milliseconds)
(define _seconds current-seconds)
(define _client-ip (lambda (port) (let-values (((x y) (tcp-addresses port))) y)))
(define _atomic-invoke (lambda (f) (if (thread-cell-ref ar-sema-cell) (ar-apply f (quote ())) (begin (thread-cell-set! ar-sema-cell #t) (let ((ret (call-with-semaphore ar-the-sema (lambda () (ar-apply f (quote ())))))) (thread-cell-set! ar-sema-cell #f) ret)))))
(define _dead (lambda (x) (tnil (thread-dead? x))))
(define _ssyntax (lambda (x) (tnil (ssyntax? x))))
(define _ssexpand (lambda (x) (if (symbol? x) (expand-ssyntax x) x)))
(define _seval (lambda (x) (eval (ac-denil x) arc-ns)))
(define _quit exit)
(define _connect-socket (lambda (host port) (let-values (((in out) (tcp-connect host port))) (list in out))))
(define _flush-socket (lambda (s) (flush-output s)))
(define _bit-and bitwise-and)
(define _bit-or bitwise-ior)
(define _bit-not bitwise-not)
