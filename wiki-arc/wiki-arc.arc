  ...
  (let (help* sig source-file* wiki add-wiki
    ...
    (def wiki (op name data meta css   args req)
      ...
      (with (data data:urlencode
        ...
        (let (talk-page-p talk-page article-page
          ...
          (=
             ...
             (fn (p rv action)
               (w/html-tags
                 ...
                 ('.topbar
                   ...
                   ('span ('(a href (+ "?title=" (urlencode (article-page p))))
                            (if (~talk-page-p p) (pr "<b>"))
                            (pr "article")
                            (if (~talk-page-p p) (pr "</b>"))))
                   ('span ('(a href (+ "?title=" (urlencode (talk-page p))))
                            (if (talk-page-p p) (pr "<b>"))
                            (pr "discussion")
                            (if (talk-page-p p) (pr "</b>"))))
                   ('span ('(a href (+ "?title=" (urlencode p) "&action=edit"
                                       (if rv (+ "&rv=" (string rv)) "")))
                            (if (is action 'edit) (pr "<b>"))
                            (pr "edit")
                            (if (is action 'edit) (pr "</b>"))))
                   ('span ('(a href (+ "?title=" (urlencode p) "&action=hist"))
             ...
             (fn (p)
               (*wiki-page (+ "Revision history of "
                              (_->space p) " - " name) css
                 (add-ons p (head-rv (or meta.p "")) 'history)
                 ('.main
                   (if meta.p
                       ('ul
