...
(suite errors
       (suite ccc
              ccc-returns (assert-same "bailout value"
                                       (ccc (fn (esc) (esc "bailout value") 42)))
              catch-expands-to-ccc-return-val (assert-same 3
                                                           (test-find-char "abcdefg" #\d))
              catch-expands-to-ccc-return-nil (assert-nil (test-find-char "abcdefg" #\z))
              continuation-passing-style (assert-t ((fn ((cps* cpsplus cps-sqrt cps-pyth))
              ...
              co-routines ; adapted from http://community.schemewiki.org/?call-with-current-continuation
       ...
       (suite protect
              simple (assert-same "protected-foo"
                                  ((fn (x)
                                       (protect (fn () (/ 1 2)) (fn () (assign x "protected-foo")))
                                       x) nil))
              continuations (assert-same "problem-free"
                                         (tostring (catch (after (throw pr!problem) pr!-free))))
              co-routine-pair (assert-same '(proc-A-start inner-A-start-5 pre-ccc-my-b proc-B-start after-ccc-my-b-5
       ...
       (suite error-handling
              no-error (assert-same 42
                                    (on-err (fn (ex) "got error")
                                            (fn () (* 6 7))))
              error-exists (assert-same "got error /: division by zero"
                                        (on-err (fn (ex) (+ "got error " (details ex)))
                                                (fn () (/ 42 0))))
              explicit-error (assert-same "got error we can also throw our own exceptions"
                                          (on-err (fn (ex) (+ "got error " (details ex)))
