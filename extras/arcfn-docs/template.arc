...
(def op (args)
  (with (tag nil tags nil operation nil arglist nil desc nil testlist nil)
    ...
    (if (no (intags 'nolink tags))
      (prn "<a target='CODE' href='/src/"  (subst "%2f" "%" (codelinkencode (string operation))) ".html'><img src='../docs/code.gif' title='code'/></a>"))
    (if (intags 'mac tags) (prn "<img src='../docs/macro.gif' title='Macro'/>"))
    (if (intags 'op tags) (prn "<img src='../docs/foundation.gif' title='Foundation'/>"))
    (if (intags 'def tags) (prn "<img src='../docs/proc.gif' title='Procedure'/>"))
    (if (intags 'var tags) (prn "<img src='../docs/var.gif' title='Variable'/>"))
    (if (intags 'destructive tags) (prn "<img src='../docs/destructive.gif' title='Destructive'/>"))
    (if (intags 'predicate tags) (prn "<img src='../docs/predicate.gif' title='Predicate'/>"))
